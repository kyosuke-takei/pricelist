...
const SEARCH_URLS = [
  'https://snkrdunk.com/search?searchCategoryIds=6%2F26&brandIds=weis-schwarz&sort=hottest',
  'https://snkrdunk.com/search?searchCategoryIds=6%2F27&brandIds=weis-schwarz',
  'https://snkrdunk.com/search?searchCategoryIds=6%2F28&brandIds=weis-schwarz',
];

const CACHE_FILE = 'weiss_cache.json';

const LABEL_SHRINK_ON  = 'ヴァイス BOX シュリンクあり';
const LABEL_SHRINK_OFF = 'ヴァイス BOX シュリンクなし';
const LABEL_PACK       = 'ヴァイス パック';
const LABEL_DECK       = 'ヴァイス デッキ';
const LABEL_PROMO      = 'ヴァイス プロモ';
const LABEL_OTHER      = 'ヴァイス その他';
...
function classifyByName(name) {
  ...
  // デッキ（ヴァイスはストラクチャーデッキが主流）
  if (n.includes('デッキ') || nl.includes('deck') || n.includes('ストラクチャー') ||
      n.includes('スターター') || n.includes('構築済み') || n.includes('プリコン')) return LABEL_DECK;
  ...
  // BOX → デフォルトはシュリンクあり
  if (nl.includes('box') || n.includes('ボックス')) return LABEL_SHRINK_ON;

  // セット（複数パック封入）→ BOX扱い
  if (n.includes('セット') && (n.includes('パック') || n.includes('コレクション') || n.includes('BOX'))) return LABEL_SHRINK_ON;
...
function classifyByPage(name, pageText) {
  ...
  if (t.includes('デッキ') || t.includes('ストラクチャー') || t.includes('構築済み')) return LABEL_DECK;
  ...
  if (t.includes('パック')) return LABEL_PACK;
  return LABEL_OTHER;
}
...
(async () => {
  ...
  console.log(`[ヴァイス] ラベルキャッシュ読み込み: ${cachedCount}件\n`);
